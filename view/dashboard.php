                ...
                <div class="row">  
                    <div class="col-sm-4 offset-sm-5">          
                        <a v-show="!(adding || editing)" v-on:click="addProduct(true)" 
                            href="#" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Crear producto</a><br><br>
                        <a v-show="!(adding || editing)" data-target="#myModal" data-toggle="modal"
                            href="#myModal" class="btn btn-danger btn-lg active" role="button" style="margin-left:-10px" aria-pressed="true">Vender producto</a>
            ...
            <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-body">
                        <select class="form-control" name="optionalPoint" v-model="idOpional">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="closeModal" class="btn btn-primary" v-on:click="sellProduct()" >Vender</button>
                    </div>
                    </div>
                </div>
            </div>
    ...
    methods: {
        ...
        sellProduct: function () {
            ...
            if (this.idOpional == '') {
                Swal.fire({
                    title: 'Error!',
                    text: 'Por escoja un id de los productos para vender',
                    icon: 'error',
                    confirmButtonText: 'Aceptar'
                });
            }else{
                var idOptional = this.idOpional;
                var productToSell = this.products.find(x => x.id == idOptional)
                var stock = productToSell.stock;
                if (stock == 0){
                    Swal.fire({
                    title: 'Error!',
                    ...
                    stock = stock - 1;
                    var data = {id: idOptional, stock: stock };
                    data = JSON.stringify(data);
                    console.log(data);

                    $.ajax({
                        url: './class/product-optional.php',
                        data: {'information': data},
                        type: 'Post',
                        async : false,
                        success: function (result) {
                            ...
                            Swal.fire({
                                title: 'Éxito!',
                                html: 'Producto vendido exitosamente',
                                icon: 'success',
                                confirmButtonText: 'Aceptar'
                            });
